      ...
      <section className="rounded-2xl border border-border bg-card p-6">
        ...
        <div className="mt-5 space-y-4">
          ...
          <div>
            ...
            <Textarea value={seo.description} onChange={(e) => setSeo((s) => ({ ...s, description: e.target.value }))} />
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <SwitchRow label="Allow search indexing" hint="robots: index, follow" checked={seo.index} onChange={() => setSeo((s) => ({ ...s, index: !s.index }))} />
            <SwitchRow label="Include in sitemap.xml" hint="Auto-generated at /sitemap.xml" checked={seo.sitemap} onChange={() => setSeo((s) => ({ ...s, sitemap: !s.sitemap }))} />
          </div>
        </div>
      </section>

      <button type="submit" className="rounded-full bg-ink px-6 py-3 text-sm font-medium text-paper hover:bg-gold hover:text-white">
        Save settings
      </button>
    </form>
  );
