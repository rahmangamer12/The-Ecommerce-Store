...
export function SocialAuth({ next = "/account" }: { next?: string }) {
  ...
  async function signIn(provider: Provider) {
    ...
    if (!supabase) {
      toast.error("Social login not ready", {
        description: "Add Supabase keys and enable the provider in its dashboard.",
      });
      return;
    }
    setLoading(provider);
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <button
          ...
          className="flex h-11 items-center justify-center gap-2 rounded-full border border-border bg-card text-sm font-medium transition-colors hover:border-ink/40 disabled:opacity-60"
        >
          <GoogleIcon />
